...
void X11Context::runLoop(DrawingBase *drawing)
{
    ...
    while (run)
    {
        ...
        // Mouse Button Down
            ...
            drawing->mouseButtonDown(this, e.xbutton.button, e.xbutton.x,
                                     e.xbutton.y);

        // Mouse Button Up
        else if (e.type == ButtonRelease)
            drawing->mouseButtonUp(this, e.xbutton.button, e.xbutton.x,
                                   e.xbutton.y);

        // Mouse Move
        else if (e.type == MotionNotify)
            drawing->mouseMove(this, e.xmotion.x, e.xmotion.y);
...
{
    XDrawArc(display, window, graphics_context, x - radius, y - radius,
             radius * 2, radius * 2, 0, 360 * 64);
    XFlush(display);
}

// void X11Context::drawTriangle(int x, int y, unsigned int radius)
// {
// 	XDrawPo
// 	XFlush(display);
// }
